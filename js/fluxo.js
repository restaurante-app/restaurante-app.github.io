...
(function () {
  ...
  const LS_MODO = 'pari.fluxo.modo';
  const MODOS = ['PASSOU', 'COMPROU'];
  ...
  function telaContar(view) {
    ...
    let modo = localStorage.getItem(LS_MODO) === 'COMPROU' ? 'COMPROU' : 'PASSOU';

    const seg = P.UI.seg(MODOS.map(m => ({ v: m, rotulo: m, cor: COR[m] })), modo, v => {
      modo = v;
      try { localStorage.setItem(LS_MODO, v); } catch (e) { /* ok */ }
      desenhar();
    }, 'seg-modo');
    ...
    function desenhar() {
      ...
      elRot.textContent = modo === 'PASSOU' ? 'passaram nesta contagem' : 'compraram nesta contagem';
      desenharRelogio();
      desenharNumeros();
    }
    ...
    return {
      ...
      onDados(t) { if (t.has('contagens')) desenharNumeros(); },
    };
  }
  ...
  function calcular(periodo, semana) {
    ...
    P.Store.all('contagens').forEach(c => {
      ...
      const k = c.dia_operacional + '|' + c.faixa_hora;
      let r = porDia.get(k);
      if (!r) porDia.set(k, r = { dia: c.dia_operacional, faixa: c.faixa_hora, PASSOU: 0, COMPROU: 0, temP: false, temC: false });
      r[c.modo] += +c.quantidade || 0;
      if (c.modo === 'PASSOU') r.temP = true; else r.temC = true;
    });
    const porFaixa = new Map();
    porDia.forEach(r => {
      let f = porFaixa.get(r.faixa);
      if (!f) porFaixa.set(r.faixa, f = { faixa: r.faixa, sP: 0, nP: 0, sC: 0, nC: 0, cP: 0, cC: 0 });
      if (r.temP) { f.sP += r.PASSOU; f.nP++; }
      if (r.temC) { f.sC += r.COMPROU; f.nC++; }
      if (r.temP && r.temC) { f.cP += r.PASSOU; f.cC += r.COMPROU; }
    });
    const linhas = [...porFaixa.values()].map(f => ({
      faixa: f.faixa,
      passou: f.nP ? f.sP / f.nP : null, diasP: f.nP,
      comprou: f.nC ? f.sC / f.nC : null, diasC: f.nC,
      conv: f.cP > 0 ? f.cC / f.cP * 100 : null,
    })).sort((a, b) => P.Dia.ordemFaixa(a.faixa) - P.Dia.ordemFaixa(b.faixa));
    let tP = 0, tC = 0;
    porDia.forEach(r => { if (r.temP && r.temC) { tP += r.PASSOU; tC += r.COMPROU; } });
    return {
      dias, linhas, porDia,
  ...
  function telaAnalise(view) {
    ...
    function desenhar() {
      ...
      corpo.appendChild(h('div', { class: 'an-base' },
        base + ' · média dos ' + r.diasComDado.size + (r.diasComDado.size === 1 ? ' dia' : ' dias') + ' com contagem',
        r.dias.length ? ' (' + P.Dia.rotuloCurto(r.dias[r.dias.length - 1]) + ' a ' + P.Dia.rotuloCurto(r.dias[0]) + ')' : ''));
      if (!r.linhas.length) {
        corpo.appendChild(P.UI.vazio('Sem contagens neste período. Use a tela Contar — cada toque entra aqui.'));
        return;
      }
      ...
      corpo.appendChild(h('div', { class: 'an-kpis' },
        kpi('Conversão geral', P.pct(r.convGeral, 0), 'comprou ÷ passou'),
        kpi('Pico (' + (rank === 'COMPROU' ? 'compra' : 'passagem') + ')', d.top[0] != null ? P.Dia.rotuloFaixa(d.top[0]) : '—', d.top.length ? 'maior média' : ''),
        kpi('Dias com dado', String(r.diasComDado.size), 'no período')));

      const legenda = h('div', { class: 'an-leg' },
        h('span', { class: 'lg lg-p' }, 'Passou'), h('span', { class: 'lg lg-c' }, 'Comprou'), h('span', { class: 'lg-conv' }, 'conversão'));
    ...
    function exportar(r) {
      ...
      const csv = ['dia_operacional;dia_semana;faixa;passou;comprou;conversao_pct'].concat(linhas.map(l => [
        l.dia, P.Dia.nomeSemana(l.dia), P.Dia.rotuloFaixa(l.faixa),
        l.temP ? l.PASSOU : '', l.temC ? l.COMPROU : '',
        l.temP && l.temC && l.PASSOU > 0 ? P.num(l.COMPROU / l.PASSOU * 100, 1) : '',
      ].join(';'))).join('\r\n');
    ...
    desenhar();
    return { onDados(t) { if (t.has('contagens')) desenhar(); } };
  }
  ...
  P.Fluxo = { reconciliar, calcular, totais, _blocos: () => blocos };
})();
